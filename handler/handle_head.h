 ...
 * handle_head.h -- This file contains the initialization
 *                  routines for the http server.
 * 
 * Marcelo Feitoza Parisi 
 */

void handle_head(void* threadfd, char* rcv_buf, char* theirip) {
	...
	if(strlen(req) == 1 && flag_slash != 1) {
		...
		/* Logging */
		log_access(ACCESS_LOG, theirip, "HEAD", req, 400, content_size);
		log_error(ERROR_LOG, theirip, "HEAD", req, 400, content_size);
	...
	/* Not a bad request */
		...
		if((filedes = open(newreq, O_RDONLY)) != -1) {
			...
			/* Logging */
			log_access(ACCESS_LOG, theirip, "HEAD", req, 200, (int)file_properties.st_size);
		...
		/* Error opening the file */
			...
			/* Logging */
			log_access(ACCESS_LOG, theirip, "HEAD", req, 404, content_size);
		}
